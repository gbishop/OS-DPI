...
export class Logger extends TabPanel {
  ...
  init() {
    this.DB = new DB(this.props.dbName);
...
class DB {
  ...
  #open(event, upgradeNeeded) {
    ...
    if (upgradeNeeded) {
      let eventStore = this.#DB.createObjectStore("event", {
        keyPath: "timestamp",
      });
    ...
    log(
      `'${this.name}' database successfully ${
        upgradeNeeded ? "created" : "opened"
      }!`
    );
  }
  ...
  async clear() {
    ...
    request.onsuccess = (e) =>
      log(`'${this.name}' database successfully cleared!`);
  ...
  async save(type = "csv") {
    ...
    rows.forEach(
      (row) =>
        (row.sheetName = row.sheetName != "undefined" ? row.sheetName : "event")
    );
  ...
  async #fetchRows(type = "csv") {
    ...
    let query = request.objectStore("event").index("by-timestamp");
      ...
      query.openCursor().onsuccess = (e) => {
        ...
        if (cursor) {
          ...
          for (let event of events) {
            ...
            (type.match(/csv/i) ? columns : Object.keys(event.value)).forEach(
              (column) => (row[column] = event.value[column] || "")
            );
